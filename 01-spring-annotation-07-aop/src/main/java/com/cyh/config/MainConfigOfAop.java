 ...
 * 		本质是利用 @Import(AspectJAutoProxyRegistrar.class) 给容器中导入 AspectJAutoProxyRegistrar
 * 			利用 AspectJAutoProxyRegistrar 自定义给容器中注册bean，BeanDefinition
 * 			internalAutoProxyCreator = AnnotationAwareAspectJAutoProxyCreator *
 * 		一句话就是：给容器中注册一个 AnnotationAwareAspectJAutoProxyCreator
 *
 * 2、 AnnotationAwareAspectJAutoProxyCreator 的层次关系：
 ...
 * 			6）、注册 BeanPostProcessor，实际上就是创建 BeanPostProcessor 对象并保存在容器中
 ...
 * 				实际上是调用 beanFactory.addBeanPostProcessor(postProcessor) 方法
 *
 *      =======以上是创建和注册 AnnotationAwareAspectJAutoProxyCreator 的过程========
 *
 * 			AnnotationAwareAspectJAutoProxyCreator 继承自 InstantiationAwareBeanPostProcessor
 * 		4）、finishBeanFactoryInitialization(beanFactory) 完成 BeanFactory 初始化工作；创建剩下的单实例bean
 ...
 * 			2）、否则调用 super.shouldSkip，永远返回false
 * 2）、创建对象
 *      postProcessAfterInitialization()
 * 		return wrapIfNecessary(bean, beanName, cacheKey);//如果需要包装的情况下
 * 		1）、获取当前bean的所有增强器（通知方法）  【Object[] specificInterceptors】
 * 			1、找到所有候选的增强器（找哪些通知方法是需要切入当前bean方法的）
 * 			2、获取到能在bean使用的增强器
 * 			3、给增强器排序
 * 		2）、保存当前bean在 advisedBeans 中，表示当前Bean已经被增强处理过了
 ...
 * 		3）、AnnotationAwareAspectJAutoProxyCreator 是一个 InstantiationAwareBeanPostProcessor 的后置处理器
 * 		4）、容器的创建流程：
 ...
 * 					如果是：将切面的通知方法包装成增强器（Advisor），给业务逻辑组件创建一个代理对象（cglib）
 * 		5）、执行目标方法：
 * 			1）、代理对象执行目标方法
